...
class YOLOClassificationTrainer:
    ...
    This class handles dataset preparation, model training, and
    inference for YOLO classification tasks. It manages directory
    structures compatible with the Ultralytics YOLO framework and
    provides methods for the complete training pipeline.

    The class assumes external configuration of auxiliary attributes
    like `yolo_classes_path`, `yolo_notes_path`, and
    `test_percentual_divisor` for seamless integration with existing
    projects.

    Attributes:
        image_folder: Tuple containing (path_folder, name_folder)
            where path_folder is the absolute or relative path to
            images and name_folder is the class label used in dataset
            structure.
        percentual_data_divisor: Percentage of data allocated to test
            set.
        predict_object: Input object for prediction (image path,
            folder path, or list of paths).
    ...
    def image_folder(self, folder: tuple[str, str] | None) -> None:
        ...
        Args:
            folder: Tuple of (path_folder, name_folder) where
                path_folder is the source directory and name_folder
                is the class label, or None to unset.
    ...
    def percentual_data_divisor(self) -> int | float | None:
        ...
        Returns:
            Percentage of data allocated to test set (e.g., 20 for
            20%), or None if not configured.
        """
    ...
    def predict_object(self) -> Any:
        ...
        Returns:
            Input object for YOLO prediction. Can be a single image
            path, directory path, or list of paths.
        """
    ...
    def slicing_dataset_for_training(self) -> None:
        """Prepare dataset for training by splitting into sets.
        ...
        Copies auxiliary files (classes.txt, notes.json) if their
        paths are configured via `yolo_classes_path` and
        `yolo_notes_path` attributes.

        Splits images from `image_folder` into train and test
        directories based on `percentual_data_divisor` or
        `test_percentual_divisor`.

        Raises:
            FileNotFoundError: If source directories don't exist.
            PermissionError: If lacking write permissions for
                destination.
        ...
        Notes:
            This method expects external configuration of:
                - self.yolo_classes_path (optional): Path to
                  classes.txt
                - self.yolo_notes_path (optional): Path to
                  notes.json
                - self.test_percentual_divisor (optional):
    ...
    def training_yolo_model(
        ...
        Loads a pre-trained YOLO model and fine-tunes it on the
        prepared dataset. Expects a `dataset.yaml` configuration file
        at `datasets/dataset_YOLO/dataset.yaml`.
    ...
    def predict_yolo_model(
        ...
        Performs predictions on the input configured via
        `predict_object`. Results can be saved to disk and include
        predicted classes and confidence scores.

        Args:
            yolo_model: Path to trained model weights. Default path
                assumes Ultralytics default output structure for
                classification.
            save_predict: Whether to save prediction results and
                visualizations to disk.
            img_size: Input image size for inference (square
                dimension in pixels).
            predict_confidence: Minimum confidence threshold for
                predictions. Predictions below this threshold may be
                filtered.

        Returns:
            Prediction results object from Ultralytics YOLO
            containing predicted classes, confidence scores, and
            other inference data.
