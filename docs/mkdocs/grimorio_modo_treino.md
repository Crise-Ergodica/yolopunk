...
| ----------------- | ---------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `model`           | `str`                  | `None`   | Especifica o arquivo do modelo para treinamento. Aceita um caminho para um arquivo `.pt` pré-treinado ou um arquivo de configuração `.yaml`. Essential para definir a estrutura do modelo ou inicializar os pesos.                               |
| `data`            | `str`                  | `None`   | Caminho para o arquivo de configuração do conjunto de dados (ex: `coco8.yaml`). Contém parâmetros específicos do dataset, incluindo caminhos para treinamento e validação, gnomes de classes e número de classes.                                |
...
| `project`         | `str`                  | `None`   | Gnome do diretório do projeto onde as saídas de treinamento são salvas. Permite armazenamento organizado de diferentes experimentos.                                                                                                             |
| `name`            | `str`                  | `None`   | Gnome da execução de treinamento. Usado para criar um subdiretório dentro da pasta do projeto, onde logs e saídas são armazenados.                                                                                                               |
| `exist_ok`        | `bool`                 | `False`  | Se `True`, permite sobrescrever um diretório de projeto/gnome existente. Útil para experimentação iterativa sem limpar manualmente saídas anteriores.                                                                                            |
| `pretrained`      | `bool` ou `str`        | `True`   | Determina se o treinamento deve começar a partir de um modelo pré-treinado. Pode set booleano ou caminho string para modelo específico. Melhora eficiência e desempenho.                                                                         |
...
| `multi_scale`     | `bool`                 | `False`  | Ativa treinamento multi-escala aumentando/diminuindo `imgsz` por até fator de 0.5. Treina modelo para set mais preciso com múltiplos tamanhos.                                                                                                   |
| `cos_lr`          | `bool`                 | `False`  | Utilize scheduler de learning rate de cosseno, ajustando a taxa seguindo curva de cosseno ao longo das épocas.                                                                                                                                   |
...
| `fraction`        | `float`                | `1.0`    | Especifica fração do dataset a set usada para treinamento. Permite treinamento em subconjunto, útil para experimentos ou recursos limitados.                                                                                                     |
...
| `momentum`        | `float`                | `0.937`  | Fator de memento para SGD ou beta1 para otimizadores Adam, influencia incorporação de gradientes anteriores na atualização actual.                                                                                                               |
| `weight_decay`    | `float`                | `0.0005` | Thermo de regularização L2, penalizando pesos grandes para evitar overfitting.                                                                                                                                                                   |
...
| `overlap_mask`    | `bool`                 | `True`   | Determina se máscaras de objeto devem set mescladas em máscara única ou mantidas separadas. Em sobreposição, máscara menor sobrepõe à maior.                                                                                                     |
